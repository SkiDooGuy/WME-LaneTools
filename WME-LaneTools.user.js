...
(
    function main() {
      ...
      const LANETOOLS_VERSION = "" + GM_info.script.version,
            ...
            LI_UPDATE_NOTES =
                "<b>NEW:</b><br>\n<b>FIXES:</b><br>Copy Toolbar comes up into Map Window and Hidden<br>\n",
            LANETOOLS_DEBUG_LEVEL = 0x1, configArray = {}, RBSArray = {failed : false},
            IsBeta = location.href.indexOf("beta.waze.com") !== -0x1, TRANSLATIONS = {
              default : {
                enabled : "Enabled",
                disabled : "Disabled",
      ...
      function initLaneTools() {
        ...
        const ltCSS =
                [
                  ...
                  // "#lt-toolbar-container {position:fixed;left:500px;top:500px;z-index:1004;height:150px;width:70px;visibility:hidden;}",
        ...
        const $ltToolbar = $(constantStrings.divStr);
        $ltToolbar.html([
        ...
        if (_pickleColor >= 0x0) {
          ...
          $("<style>" + ltCSS + "</style>").appendTo("head");
          $("#map").append($ltToolbar.html());
      ...
      function processOptionSettings() {
        ...
        setChecked("lt-DevToolsEnable", LtSettings.DevToolsEnable);
        ...
        function setChecked(checkboxId, propertyValue) { $("#" + checkboxId).prop("checked", propertyValue); }

        function setCSSBorder(idName, color) {
          const idSelector = $("#" + idName);
          idSelector.attr("value", color);
          idSelector.css("border", "2px solid ${color}");
        }
      }
      ...
      async function setupOptions() {
        ...
        if (_pickleColor > 0x1) {
          ...
          _.each(RBSArray, (argument) => {
            if (argument[0] === seaPickle.userName) {
              argument[1] === "1" && (isRBS = true);
              argument[2] === "1" && (allowCpyPst = true);
            }
          });
          if (isRBS) {
      ...
      function configureDirectionLanesHTML() {
        ...
        $('<div class="lt-add-Width fwd">2</div>').css({
          padding : "5px 5px 10px",
          margin : "0px",
        });
      ...
      function populateNumberOfLanes() {
        ...
        if (fwdLanesSelector.find(".direction-lanes").children().length > 0x0 && !getId("lt-fwd-add-lanes")) {
          let addLanesItem = $(
                  '<div style="cursor:pointer;display:inline-flex;flex-direction:row;justify-content:space-around;margin-top:4px;" id="lt-fwd-add-lanes" />'),
        ...
        if (revLanesSelector.find(".direction-lanes").children().length > 0x0 && !getId("lt-rev-add-lanes")) {
          let reverseLaneDiv = $(
                  '<div style="cursor:pointer;display:inline-flex;flex-direction:row;justify-content:space-around;margin-top:4px;" id="lt-rev-add-lanes" />'),
      ...
      function scanSegments(segmentList, performHeuristicsCheck) {
        ...
        _.each(segmentList, (segmentObj) => {
          if (onScreen(segmentObj, minZoomLevel)) {
            const featureAttributes = segmentObj.getAttributes() // getFeatureAttributes();
      ...
      function setTurns(directionClass, numLanes) {
        ...
        let lanesWithGuidance = new Array(numLanes).fill(0);
        for (let idx = 0; idx < turnLaneEditTop.length; ++idx) {
          ...
          for(let i = 0 ; i< numLanes; ++i) {
              let turnCheckBox = checboxesElems[i].getElementsByTagName("wz-checkbox");
              if(turnCheckBox.checked !== undefined && turnCheckBox.checked === true) lanesWithGuidance[i] = 1;
          }
        }
        let numberofGuidedLanes = lanesWithGuidance.reduce((guidanceCounter, currentValue) => guidanceCounter + currentValue, 0);
        if (numberofGuidedLanes !== numLanes) {
          const clickSaveTurnsEnabled = getId("lt-ClickSaveTurns").checked;
          ...
          for (let idx = 0x0; idx < turnLaneEditTop.length; idx++) {
            ...
            if (turnCheckBoxes && turnCheckBoxes.length > 0x0) {
              if (turnLaneEditTopContainer.getElementsByClassName(leftTurnAngleClass).length > 0x0 &&
                ...
                if (clickSaveTurnsEnabled && turnCheckBoxes[0].checked === false) {
                ...
                if (clickSaveTurnsEnabled && turnCheckBoxes[turnCheckBoxes.length - 1].checked === false) {
                  rightTurnEnabled = true;
                  turnCheckBoxes[turnCheckBoxes.length - 1].click();
                }
              }
            }
          }
          for (let idx = 0x0; idx < turnLaneEditTop.length; idx++) {
            ...
            const allStraightEnabled = getId("lt-ClickSaveStraight").checked;
            if (turnLanesContainer.getElementsByClassName("angle-0").length > 0x0)
              for (let idx = 0x0; idx < directionCheckBoxes.length; idx++) {
                if (directionCheckBoxes[idx].checked === false) {
                  if (idx === 0x0 && (allStraightEnabled ||
                      (!allStraightEnabled && ((!isLeftHand && !leftTurnEnabled) || (isLeftHand && !rightTurnEnabled)))
                  ))
                    directionCheckBoxes[idx].click();
                  else {
                    if (idx === directionCheckBoxes.length - 0x1 &&
                        (allStraightEnabled ||
                            (!allStraightEnabled && ((!rightTurnEnabled && !isLeftHand) || (isLeftHand && !leftTurnEnabled)))))
                      directionCheckBoxes[idx].click();
      ...
      function copyLaneInfo(nodeName) {
        ...
        const selectedFeatures = W.selectionManager.getSelectedFeatures(),
              ...
              featureAttributes = featureObject.getAttributes() // getFeatureAttributes(),
        ...
        for (let segIdx = 0x0; segIdx < attachedSegmentIDs.length; segIdx++) {
          ...
          let _0x56f831 = segmentObject.getAttributes(), geometryComponents = segmentObject.geometry.components, // getFeatureAttributes(), geometryComponents = segmentObject.geometry.components,
      ...
      function pasteLaneInfo(nodeName) {
        ...
        const selectedFeatures = W.selectionManager.getSelectedFeatures(),
              ...
              components = featureObject.geometry.components, featureAttributes = featureObject.getAttributes() // getFeatureAttributes(),
