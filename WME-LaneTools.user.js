...
(function
     main() {
       ...
       const LANETOOLS_VERSION = "" + GM_info.script.version,
             ...
             configArray = {}, RBSArray = {failed : false}, IsBeta = location.href.indexOf("beta.waze.com") !== -0x1,
             TRANSLATIONS = {
               default : {
                 enabled : "Enabled",
                 disabled : "Disabled",
         ...
         for (const actionValue in W.accelerators.Actions) {
           ...
           if (groupValue === "wmelt") {
             ...
             if (LtSettings[actionValue] !== shortCutString) {
               ...
               console.log("LaneTools: Stored shortcut " + actionValue + ": " + LtSettings[actionValue] +
                           "changed to " + shortCutString);
               break;
             }
           }
         }
       ...
       function setupAutoFocusLanes(flagsObject) {
         if (getId("lt-AutoFocusLanes").checked) {
           ...
           else if (revLanesSelector.find(editRegionSelectors).children().length > 0x0 &&
       ...
       function highlightSegment(segGeom, segDirection, highlightEnabled, highlightLabelsEnabled, forwardLaneCount,
                                 reverseLaneCount, tioAndLIO, turnGuidanceMode, lanesMissingConfiguration,
                                 heurCandidate, applyHeuristics) {
