...
function laneToolsBootstrap(tries = 0) {
    if (W?.map && W.model && W.loginManager.user && $ && WazeWrap.Ready) {
        initLaneTools();
    } else if (tries < 500) {
        setTimeout(() => {
...
function initLaneTools() {
    ...
    const proceedReady = _pickleColor >= 0;
    ...
    if (connected) {
        _.each(configArray, (serverKey) => {
            for (const k in serverKey) {
                if (Object.prototype.hasOwnProperty.call(serverKey, k)) {
...
function lanesTabSetup() {
    ...
    function hoverNodeTo() {
        W.model.nodes.getObjectById(W.selectionManager.getSegmentSelection().segments[0].attributes.toNodeID).attributes
            .geometry;
    ...
    function colorCSDir() {
        ...
        const fwdConfig = checkLanesConfiguration(seg, fwdNode, fwdNode.attributes.segIDs, seg.attributes.fwdLaneCount);
        const revConfig = checkLanesConfiguration(seg, revNode, revNode.attributes.segIDs, seg.attributes.revLaneCount);

        if (fwdConfig[4] > 0) {
            const csColor = fwdConfig[4] === 1 ? LtSettings.CS1Color : LtSettings.CS2Color;
        ...
        if (revConfig[4] > 0) {
            const csColor = revConfig[4] === 1 ? LtSettings.CS1Color : LtSettings.CS2Color;
    ...
    function formatLanesTab(clickTab = false, tries = 0) {
        if ($(".tabs-labels > div:nth-child(3)", $(".segment-edit-section > wz-tabs")[0].shadowRoot).length > 0) {
            ...
            if (clickTab) {
                ...
                waitForElementLoaded(".lanes-tab").then((elm) => {
                    $(".tabs-labels > div:nth-child(3)", $(".segment-edit-section > wz-tabs")[0].shadowRoot).trigger(
                        "click"
                    );
                });
            }
...
function applyName(geo, fwdLnsCount, revLnsCount) {
    const nameGeo = geo.clone();
    const laneNum = `${fwdLnsCount} / ${revLnsCount}`;
    const lnLabel = new OpenLayers.Feature.Vector(nameGeo, {
        labelText: laneNum,
...
function highlightSegment(
    ...
    if (geo.components.length > 2) {
        const geoLength = geo.components.length;
        ...
        if (direction === Direction.FORWARD) {
            const newString = buildGeoComponentString(geo, fwdPoint, geoLength);
            if (applyDash) {
            ...
            const newString = buildGeoComponentString(geo, 0, revPoint);
            if (applyDash) {
        ...
        if (applyLabels && (Direction.FORWARD || fwdLnsCount === 0)) {
            if (geoLength % 2) {
                ...
                const p0 = geo.components[revPoint - 1];
                const p1 = geo.components[fwdPoint];
                //                let newPoint = new OpenLayers.getOLGeometry().Point((p0.x + p1.x) / 2, (p0.y + p1.y) / 2);
                const newPoint = new OpenLayers.Geometry.Point((p0.x + p1.x) / 2, (p0.y + p1.y) / 2);
                applyName(newPoint, fwdLnsCount, revLnsCount);
            }
        }
    } else {
        const p0 = geo.components[0];
        const p1 = geo.components[1];
        //        let point1 = new OpenLayers.getOLGeometry().Point((p0.x + p1.x) / 2, (p0.y + p1.y) / 2);
        const point1 = new OpenLayers.Geometry.Point((p0.x + p1.x) / 2, (p0.y + p1.y) / 2);
        ...
        if (direction === Direction.FORWARD) {
            ...
            const point2 = new OpenLayers.Geometry.Point(geo.components[1].clone().x, geo.components[1].clone().y);
            const newString = new OpenLayers.Geometry.LineString([point1, point2], {});
            if (applyDash) {
                createVector(newString, `${LtSettings.ABColor}`, VectorStyle.DASH_THIN);
            }
            drawHighlight(newString, applyLioHighlight, isBad, heur, heurOverHighlight);
            ...
            //            let newString = new OpenLayers.getOLGeometry().LineString([point1, point2], {});
            const point2 = new OpenLayers.Geometry.Point(geo.components[0].clone().x, geo.components[0].clone().y);
            const newString = new OpenLayers.Geometry.LineString([point1, point2], {});
            if (applyDash) {
                createVector(newString, `${LtSettings.BAColor}`, VectorStyle.DASH_THIN);
            }
    ...
    function createVector(geoCom, lineColor, style) {
        const newVector = new OpenLayers.Feature.Vector(geoCom, {}, {});
...
function scanHeuristicsCandidates(features) {
    ...
    let count = 0;
    _.each(features, (f) => {
        if (f?._wmeObject && f._wmeObject.type === "segment") {
    ...
    function scanSegment_Inner(seg, sAtts, direction, segLength, tryRedo) {
        ...
        if (onScreen(node, zoomLevel)) {
            ...
            if (segLength <= MAX_LEN_HEUR) {
                ...
                heurCand = isHeuristicsCandidate(
                    seg,
                    node,
                    nodeSegs,
                    oppNode,
...
function isHeuristicsCandidate(
    ...
    */
    ...
    const segEndAzm = lt_getMathAzimuth_to_node(curNodeExit.attributes.id, segCandidate);
    const segBeginAzm = lt_getMathAzimuth_from_node(curNodeEntry.attributes.id, segCandidate);
    ...
    const nodeEntrySegIds = curNodeEntry.getSegmentIds();
    for (let ii = 0; ii < nodeEntrySegIds.length; ii++) {
        ...
        const ia = lt_getMathAzimuth_to_node(curNodeEntry.attributes.id, is); // absolute math azimuth
    ...
    for (let ii = 0; ii < nodeExitSegIds.length; ii++) {
        ...
        const oa = lt_getMathAzimuth_from_node(curNodeExit.attributes.id, os); // absolute math azimuth
    ...
    for (let ii = 0; ii < nodeEntrySegIds.length; ii++) {
        ...
        const ia = lt_getMathAzimuth_to_node(curNodeEntry.attributes.id, ai1); // absolute math azimuth
...
function lt_segment_length(segment) {
    const len = segment.getOLGeometry().getGeodesicLength(W.map.olMap.projection);
...
function copyLaneInfo(side) {
    ...
    for (let i = 0; i < conSegs.length; i++) {
        ...
        if (turnInfo.state === 1 && turnInfo.lanes) {
            ...
            const laneData = turnInfo.getLaneData();
...
function drawIcons(seg, node, imgs) {
    ...
    // Box coords
    const boxPoint1 = new OpenLayers.Geometry.Point(startPoint.x, startPoint.y + featDis.boxheight);
    const boxPoint2 = new OpenLayers.Geometry.Point(
        startPoint.x + featDis.boxincwidth * numIcons,
        startPoint.y + featDis.boxheight
    );
    const boxPoint3 = new OpenLayers.Geometry.Point(startPoint.x + featDis.boxincwidth * numIcons, startPoint.y);
    const boxPoint4 = new OpenLayers.Geometry.Point(startPoint.x, startPoint.y);
    ...
    const boxRing = new OpenLayers.Geometry.LinearRing(points);
    const centerPoint = boxRing.getCentroid();
    boxRing.rotate(boxRotate, centerPoint);
    const boxVector = new OpenLayers.Feature.Vector(boxRing, null, boxStyle);
    ...
    _.each(imgs, (img) => {
        ...
        const iconBoxRing = new OpenLayers.Geometry.LinearRing(iconPoints);
        iconBoxRing.rotate(boxRotate, centerPoint);
        const iconBoxVector = new OpenLayers.Feature.Vector(iconBoxRing, null, iconBoxStyle);

        LTLaneGraphics.addFeatures([iconBoxVector]);

        // Icon coords
        const arrowOrigin = iconBoxRing.getCentroid();
        const iconStart = new OpenLayers.Geometry.Point(arrowOrigin.x, arrowOrigin.y);
        let ulabel = "";
        const usize = {
            x: 0,
            y: 0,
        ...
        // Add icon to map
        const iconFeature = new OpenLayers.Feature.Vector(iconStart, null, iconStyle);
...
function displayLaneGraphics() {
    ...
    if (seg.attributes.fwdLaneCount > 0) {
        waitForElementLoaded(
            ".fwd-lanes > div.lane-instruction-to > div.instruction > div.lane-arrows > div.lane-arrow > svg > polyline"
        ).then((elem) => {
            const fwdEle = getIcons(
                $(".fwd-lanes")
                    .find(".lane-arrow")
                    .map(function () {
                        return this;
            ...
            const fwdImgs = convertToBase64(fwdEle);
    ...
    if (seg.attributes.revLaneCount > 0) {
        waitForElementLoaded(
            ".rev-lanes > div.lane-instruction-to > div.instruction > div.lane-arrows > div.lane-arrow > svg > polyline"
        ).then((elem) => {
            const revEle = getIcons(
                $(".rev-lanes")
                    .find(".lane-arrow")
                    .map(function () {
                        return this;
            ...
            const revImgs = convertToBase64(revEle);
